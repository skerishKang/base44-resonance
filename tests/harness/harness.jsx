...
function Harness(){const[language,setLanguage]=useState("en");const adapter=useMemo(()=>createInMemoryWatchTreeAdapter(),[]);const copy=getWatchTreeCopy(language);const text=getCopy(language);const changeLanguage=(nextLanguage)=>setLanguage(nextLanguage);const handlePrivacy=(event)=>{event.preventDefault();navigateToElementById("watchtree-privacy-overview",{block:"start"});};return <main className="site-shell" data-test-harness="watchtree"><header className="site-header" data-testid="site-header"><a className="wordmark" href="#top" aria-label="Resonance home">Resonance</a><nav aria-label="Primary navigation"><a href="#watchtree-story">{copy.nav.story}</a><a href="#watchtree-privacy-overview" onClick={handlePrivacy}>{copy.nav.privacy}</a><LanguageSwitch language={language} onChange={changeLanguage} labels={text.language}/><button className="nav-enter" data-testid="header-enter" type="button" onClick={()=>document.querySelector("#experience")?.scrollIntoView()}>{copy.nav.enter}</button></nav></header><div id="top"><CinematicWatchTree copy={copy} onPrimary={()=>document.querySelector("#experience")?.scrollIntoView()} onPrivacy={handlePrivacy}/></div><WatchTreeExperience language={language} adapter={adapter}/></main>}
